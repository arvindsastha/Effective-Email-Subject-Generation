...
def email_body_characteristics(mail):
  #words_in_mail = mail.replace(".", " ").strip()
  number_of_sentences = mail.strip().split(".")
  ...
  #print(sentence_count, " ", word_count)
  #word_count = words_in_mail.split(" ")
...
def segrating_reply_email(email_body, inbox_identifier):
  ...
  for email in original_emails:
    ...
    for recipient in to_header:
      ...
      #print("1. ", sentence_count, " ",words_count)
      
      if sentence_count < 3 or words_count < 35:
        continue
    ...
    for recipient in recipient_list:
      ...
      sentence_count, words_count = email_body_characteristics(body)
      #print("2. ", sentence_count, " ", words_count)
...
def parse_emails_in_folder(folder_path):
  ...
  if os_exists(folder_path):
    for email_file_name in listdir(folder_path):    
      ...
      if isdir(filename):
        continue      
      with codecs.open(filename, "r", encoding='utf-8', errors='ignore') as raw_email_file:
...
def extract_conversations(folder_path):
  ...
  for employee_folder in employee_folders:
    ...
    for sent_folder in sent_folder_list:
      print(sent_folder)
      email_list, email_files = parse_emails_in_folder(sent_folder)
      i = 0
    ...
    for inbox_folder in inbox_folder_list:
      print(inbox_folder)
      email_list, emial_files = parse_emails_in_folder(inbox_folder)
      i = 0
      #print(i)
      ...
      for inbox_email in email_list:
        ...
        try:
          ...
          #print("Exception occured")
          pass
    count = count + (len(unique_body)-count)
    print(count)
    if count >= 13000:
        break
  #print("****")
  ...
  data_frame['Original Body'] = Original_body
  
  data_frame.to_csv("conversation_val.csv", index=False) 
  data_frame.to_pickle("conversation_val.pickle")
  data_frame = pk.load(open("conversation_val.pickle", "rb"))
  print(data_frame)
  
  data_frame_1['Subject'] = unique_subject
  data_frame_1['Email Body'] = unique_body
  #data_frame_1['Original Body'] = Original_body
  
  data_frame_1.to_csv("emails_and_subject_val.csv", index=False) 
  data_frame_1.to_pickle("emails_and_subject_val.pickle")
  data_frame_1 = pk.load(open("emails_and_subject_val.pickle", "rb"))
  print(data_frame_1)
  
  data_frame_2 = pd.DataFrame(list(pair_mail.items()),columns=['Pair','Email Set'])
  
  data_frame_2.to_csv("pair_and_email_list_val.csv", index=False) 
  data_frame_2.to_pickle("pair_and_email_list_val.pickle")
  data_frame_2 = pk.load(open("pair_and_email_list_val.pickle", "rb"))
  print(data_frame_2)
  ...
  data_frame_3.to_csv("from_to_email_list_val.csv", index=False) 
  data_frame_3.to_pickle("from_to_email_list_val.pickle")
  data_frame_3 = pk.load(open("from_to_email_list_val.pickle", "rb"))
  print(data_frame_3)
